...
changelog: 2026-06-15 (rev4) — open_cloze core SHIPPED (KET_R_P5/PET_R_P6 AI-gen
  được): điền-từ TỰ GÕ (fill_blank, không option). | 2026-06-13 (rev3) — B3–B7
  SHIPPED; status markers + API contract synced to implementation
  (scaffold-section, exams/scaffold, error-codes, validate-on-save,
  eligibility_reason, full preset catalogue + aiGenSupported/imageDependent).
  mc_cloze core SHIPPED (KET_R_P4/PET_R_P5 AI-gen được). B8 + remaining cores
  (matching/gapped/listening) remain ⏳.
---
...
| ⏳ **ĐỂ SAU** | AI-gen core matching/gapped/listening / cả-đề / render ảnh — amendment đợt sau. |
...
generate** — sinh nội dung theo khuôn, đè đề gốc (✅ Part MC reading + **mc_cloze
điền-từ** + **open_cloze điền-từ tự gõ**; matching/gapped/listening ⏳).
...
- **Chỉ Part `aiGenSupported=true` mới AI-gen được** (đợt này: core
  `multiple_choice` ⇒ `KET_R_P3`, `PET_R_P3`; core `mc_cloze` (điền-từ chọn) ⇒
  `KET_R_P4`, `PET_R_P5`; core `open_cloze` (điền-từ tự gõ) ⇒ `KET_R_P5`,
  `PET_R_P6`). Gửi `partCode` của Part chưa hỗ trợ (vd matching/gapped/listening)
  → **400** "chưa hỗ trợ AI-gen". Builder/scaffold dùng mọi Part.
- **mc_cloze** (điền từ — chọn): đề ra là 1 đoạn có **N chỗ trống `{{gap:N}}`**,
  mỗi câu chọn TỪ điền vào (KET 3 / PET 4 lựa chọn). FE render như MC-gap thường.
- **open_cloze** (điền từ — TỰ GÕ): 1 đoạn có **N chỗ trống `{{gap:N}}`**, mỗi câu
  là **`fill_blank`** (KHÔNG có option) — học sinh **tự gõ một từ**. Đáp án chấm
  theo **accept-list** (`question_data.correct_answers`, nhiều từ chấp nhận được;
  so khớp không phân biệt hoa/thường + trim). FE render như fill-in-the-blank
  thường (giống KET/PET open cloze trên giấy), 1 ô nhập / gap.
...
- **AI-gen core còn lại**: matching / gapped_text / listening (các Part
  `aiGenSupported=false`). Builder/scaffold/validate đã chạy cho chúng; chỉ AI-gen
  chưa. (MC reading + **mc_cloze** + **open_cloze** đã AI-gen được.)
