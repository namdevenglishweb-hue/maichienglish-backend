...
changelog: 2026-06-13 (rev3) — B3–B7 SHIPPED; status markers + API contract
  ...
  imageDependent). mc_cloze core SHIPPED (KET_R_P4/PET_R_P5 AI-gen được). B8 +
  remaining cores (matching/gapped/listening) remain ⏳.
---
...
điền-từ**; matching/gapped/listening ⏳).
...
- **Chỉ Part `aiGenSupported=true` mới AI-gen được** (đợt này: core
  `multiple_choice` ⇒ `KET_R_P3`, `PET_R_P3`; core `mc_cloze` (điền-từ) ⇒
  `KET_R_P4`, `PET_R_P5`). Gửi `partCode` của Part chưa hỗ trợ (vd matching/
  gapped/listening) → **400** "chưa hỗ trợ AI-gen". Builder/scaffold dùng mọi Part.
- **mc_cloze** (điền từ): đề ra là 1 đoạn có **N chỗ trống `{{gap:N}}`**, mỗi câu
  chọn TỪ điền vào (KET 3 / PET 4 lựa chọn). FE render như MC-gap thường (đã có).
...
- **AI-gen core còn lại**: matching / gapped_text / listening (các Part
  ...
  chưa. (MC reading + **mc_cloze** đã AI-gen được.)
