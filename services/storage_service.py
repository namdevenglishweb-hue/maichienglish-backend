...
class StorageService(ABC):
    ...
    async def upload_bytes(
        ...
        """Upload server-generated bytes directly (service-role) → public URL.

        For media produced on the server (e.g. AI image generation,
        docs/exam-image-generation §6). Validates `content_type` against
        ALLOWED_TYPES[bucket] + `len(data)` against SIZE_LIMITS[bucket].
        Extension derived from `content_type` (filename not trusted); a UUID
        is generated for the path.
        """
        ...
